...
public class ReceitaController {
    ...
    //Chamada da procedure, vai exibir apenas as receitas do paciente cujo o nome será informado
    ...
    public ResponseEntity<List<ReceitaDTO>> exibirReceitasDoPaciente(@PathVariable String nome){
        ...
        receitas.forEach(receita -> {
            dtos.add(new ReceitaDTO(receita.getId(), receita.getDataReceita(), receita.getMedicamentos(), receita.getMedicoCRM(), receita.getPacienteNome()
            ));
        });
    ...
    public ResponseEntity<List<ReceitaDTO>> exibirMedicamentos(){
        ...
        receitas.forEach(receita -> {
            ...
            dtos.add(new ReceitaDTO(receita.getId(), receita.getDataReceita(), medicamentos, receita.getMedicoCRM(), receita.getPacienteNome()
                     ));
        });
        return ResponseEntity.status(200).body(dtos);
    }
    ...
    public ResponseEntity<ReceitaDTO> exibirReceita(@PathVariable Long id){
        ...
        return ResponseEntity.status(200).body(new ReceitaDTO(receita.getId(), receita.getDataReceita(), receita.getMedicamentos(), receita.getMedicoCRM(), receita.getPacienteNome()
        ));
    }
    ...
    public ResponseEntity<ReceitaDTO> atualizar(@PathVariable Long id, @RequestBody ReceitaRequestDTO dados) {
        ...
        return ResponseEntity.ok().body(
                new ReceitaDTO(encontrado.getId(), encontrado.getDataReceita(),encontrado.getMedicamentos(), encontrado.getMedicoCRM(), encontrado.getPacienteNome()
        ));
    }
